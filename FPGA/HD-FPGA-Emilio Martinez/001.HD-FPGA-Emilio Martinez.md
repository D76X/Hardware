...
[FFT on FPGA (Part 4): Real-Time FFT with ADC Sampling, MicroBlaze, and Vivado Emilio Martinez III](https://www.youtube.com/watch?v=qti72oHhGj0)  
...
The divider can only divide S(a.b) numbers, so all inputs must be converted into S(a.b) before operation. 
The divider has three parameters. 
To ensure no precision or overflow errors come, always use these values when instantiating the IP block:

```
DATA_WIDTH = Ainput FRACTION = binput
OUT_WIDTH = ainput + binput
```

Note that with these parameters, the output format is always:

`aoutput = input + binput boutput = binput`

![Verilog Divider](./05.VerilogDivider.png)  

![Verilog Divider Example 1](./06.VerilogDividerIPExample01.png)  
